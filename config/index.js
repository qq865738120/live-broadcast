...
try {
	window;
	window.location.origin.indexOf("soukong") > -1 ?
		(env = "pro") :
		(env = "dev");
} catch (e) {}
let host;
...
const port = '81' // 开发环境端口号配置

const devConfig = {
	environment: 'local', //本地环境配为local，线上环境配为online
	host: 'http://180.180.180.190:' + port, //本地环境，与本地计算机ip保持一致
	path: '/api',
...
module.exports = {
	...
	dev: {
		...
		host: app.host.split("//")[1].split(":")[0], // can be overwritten by process.env.HOST
		// host: '192.168.10.101',
